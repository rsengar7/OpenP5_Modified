...
def prefix_allowed_tokens_fn(candidate_trie, tokenizer):
    def prefix_allowed_tokens(batch_id, sentence):
        sentence = sentence.tolist()
        trie_out = candidate_trie.get(sentence)
        
        if not trie_out:
            trie_out = list(range(tokenizer.vocab_size))  # Adjust as per your needs
...
if __name__ == "__main__":
    ...
    candidate_trie = Trie([[0] + tokenizer.encode("Beauty item {}".format(e), add_special_tokens=False) for e in candidates])
